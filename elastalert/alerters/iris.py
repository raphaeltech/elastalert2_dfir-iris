...
class IrisAlerter(Alerter):
    ...
    def make_iocs_records(self, matches):
        ...
        for record in self.iocs:
            ...
            if ioc_value is not None:
                record['ioc_value'] = str(ioc_value)
                iocs.append(record)
        return iocs
    ...
    def make_alert(self, matches):
        ...
        alert_data = {
            "alert_title": self.lookup_field(matches[0], 'kibana.alert.rule.name', None),
            "alert_description": self.lookup_field(matches[0], 'kibana.alert.rule.name', None),
            ...
            "alert_note": self.lookup_field(matches[0], 'kibana.alert.rule.description', None),
            "alert_tags": ','.join(self.lookup_field(matches[0], 'kibana.alert.rule.tags', [])),
